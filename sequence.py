...
# Global variables for cumulative data collection
cumulative_collection_active = False
cumulative_data = []  # Store all emotion scores during collection period
cumulative_data_lock = threading.Lock()  # Thread-safe access to cumulative_data
cumulative_collection_thread = None
...
def continuous_emotion_detection():
    ...
    global emotion_data, emotion_history, emotion_detection_running, model_output_streaming_active, latest_frame_data, algorithm_config, cumulative_collection_active, cumulative_data
    
    print("Starting continuous emotion detection...")
    
    # Ensure camera is opened and valid before starting
    if not camera_input.isOpened():
        print("ERROR: Camera is not opened. Cannot start emotion detection.")
        emotion_detection_running = False
        return
    
    while emotion_detection_running:
        # Verify camera is still opened (prevent switching to other sources)
        if not camera_input.isOpened():
            print("ERROR: Camera connection lost during detection.")
            break
        
        # Get frame from camera_input - always read fresh frame from webcam
        ret, frame = camera_input.read_flipped()
        ...
        # Validate frame is from camera (basic sanity check - should have reasonable dimensions)
        if frame.shape[0] < 100 or frame.shape[1] < 100:
            print("Warning: Frame dimensions seem invalid, skipping...")
            time.sleep(0.1)
            continue
        ...
        if result and result['face_detected']:
            ...
            # If cumulative collection is active, store this data point
            if cumulative_collection_active:
                with cumulative_data_lock:
                    cumulative_data.append({
                        'emotions': emotions.copy(),
                        'scores_out_of_10': convert_to_out_of_10(emotions),
                        'timestamp': time.time()
                    })
...
def initialize():
    ...
    try:
        ...
        print(f'Emotion detector models loaded')
        # Initialize camera - ensure it's locked to webcam only
   
        # Reinitialize camera to ensure we're using the webcam
        camera_input._initialize_camera()
        print(f'camera_input: {camera_input}')
        # Verify camera is actually reading from webcam (not screen)
        if camera_input is not None and camera_input.isOpened():
            ret, test_frame = camera_input.read()
            if not ret or test_frame is None:
                return jsonify({
                    'status': 'error',
                    'message': 'Camera opened but cannot read frames. Make sure webcam is not being used by another application.',
                    'processStatus': process_status,
                    'optionalMessageData': {
                        'camera_ready': False,
...
def cumulative_data_collection_thread():
    """Thread function to collect cumulative data for 30 seconds"""
    global cumulative_collection_active, cumulative_data
    
    print("Starting cumulative data collection for 30 seconds...")
    
    # Clear previous cumulative data
    with cumulative_data_lock:
        cumulative_data = []
    
    # Collect data for 30 seconds
    start_time = time.time()
    while time.time() - start_time < 30.0 and cumulative_collection_active:
        time.sleep(0.1)  # Small sleep to prevent busy waiting
    
    # Stop collection after 30 seconds
    cumulative_collection_active = False
    print(f"Cumulative data collection completed. Collected {len(cumulative_data)} data points.")

@app.route('/trigger-sequence', methods=['POST'])
def trigger_sequence():
    """Start cumulative data collection for 30 seconds"""
    global cumulative_collection_active, cumulative_collection_thread, emotion_detection_running
    
    # Check if emotion detection is running
    if not emotion_detection_running:
        return jsonify({
            'status': 'error',
            'message': 'Emotion detection is not running. Please start emotion detection first.',
            'processStatus': process_status,
            'optionalMessageData': None
        }), 400
    ...
    if cumulative_collection_active:
        return jsonify({
            'status': 'error',
            'message': 'Cumulative data collection is already active',
            'processStatus': process_status,
            'optionalMessageData': None
        }), 400
    
    # Start cumulative collection
    cumulative_collection_active = True
    
    # Start collection thread
    cumulative_collection_thread = threading.Thread(target=cumulative_data_collection_thread, daemon=True)
    cumulative_collection_thread.start()
    
    return jsonify({
        'status': 'success',
        'message': 'Cumulative data collection started. Will collect data for 30 seconds.',
        'processStatus': process_status,
        'optionalMessageData': {
...
def get_cumulative_data():
    """Get all cumulative data collected during the last collection period and stop camera output"""
    global cumulative_data, cumulative_collection_active
    
    # Stop camera output (emotion detection)
    stop_continuous_emotion_detection()
    
    with cumulative_data_lock:
        # Create a copy of cumulative data for safe access
        data_copy = [entry.copy() for entry in cumulative_data]
        collection_active = cumulative_collection_active
    
    # Calculate statistics
    total_data_points = len(data_copy)
    
    if total_data_points == 0:
        return jsonify({
            'status': 'success',
            'message': 'No cumulative data available. Start collection with /trigger-sequence',
            'data': [],
            'statistics': {
                'total_data_points': 0,
    ...
    # Calculate time span
    if data_copy:
        start_time = data_copy[0]['timestamp']
        end_time = data_copy[-1]['timestamp']
    ...
    if data_copy:
        ...
        average_emotions = {}
        average_scores_out_of_10 = {}
        
        for emotion_key in all_emotion_keys:
            scores = [entry['emotions'].get(emotion_key, 0) for entry in data_copy if emotion_key in entry['emotions']]
            scores_10 = [entry['scores_out_of_10'].get(emotion_key, 0) for entry in data_copy if emotion_key in entry['scores_out_of_10']]
            
            if scores:
                average_emotions[emotion_key] = sum(scores) / len(scores)
            if scores_10:
                average_scores_out_of_10[emotion_key] = sum(scores_10) / len(scores_10)
    else:
        average_emotions = {}
        average_scores_out_of_10 = {}
    ...
    return jsonify({
        'status': 'success',
        'message': 'Cumulative data retrieved successfully',
        'data': data_copy,
        'statistics': {
            'total_data_points': total_data_points,
            'collection_active': collection_active,
            'duration_seconds': round(duration, 2),
            'start_time': data_copy[0]['timestamp'] if data_copy else None,
            'end_time': data_copy[-1]['timestamp'] if data_copy else None,
            'average_emotions': convert_numpy_types(average_emotions),
            'average_scores_out_of_10': convert_numpy_types(average_scores_out_of_10)
        }
    })
...
def chat():
    ...
    full_prompt = f"""Based on the following emotional data from a test subject, of how their emotional state may affect humanity. Maximum 30 words:

Emotional Data:
{emotion_data_string}

Provide a detailed analysis of this emotional data and discuss potential outcomes or insights."""
